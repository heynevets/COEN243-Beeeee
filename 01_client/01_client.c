...
//    WPRINT_APP_INFO(("Sent Message=%s\n",sendMessage)); // echo the message so that the user can see something
    WPRINT_APP_INFO(("Sent Message=%s\n","Nothing")); // echo the message so that the user can see something
...
//	wiced_tcp_stream_flush(&stream);
	...
	char readBuffer[100];
	wiced_tcp_stream_read(&stream, readBuffer, 100, 2000);
	for (int m = 0; m < 100; m++){
	    if (readBuffer[m] != 0) {
	        printf("%c", readBuffer[m]);
	    }
    }
//	WPRINT_APP_INFO(("Sent Message=%s\n", readBuffer)); // echo the message so that the user can see something
...
//	{
    ...
    SET_IPV4_ADDRESS( serverAddress, MAKE_IPV4_ADDRESS( 10, 16, 230, 247) );
